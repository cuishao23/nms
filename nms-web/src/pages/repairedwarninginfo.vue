        ...
        <div class="server">
          <div>
            <div>
              ...
              <el-input v-model="device_name_s" placeholder="设备名称" class="filter-text"></el-input>
            </div>
          </div>
          <div class="warning">
            <div>
              ...
              <button class='search' @click="searchRepairedWarningsS()"></button>
              <span class="clear-filter" @click="clearFilterS()">清除筛选</span>
            </div>
          </div>
        </div>
        <div v-show="repairedserverWarningList.length === 0 && slistFlash" class="no-info-tip">
          <span class="PromptImg"></span>
          <span>没有满足条件的统计数据！</span>
        </div>
        <div v-show="repairedserverWarningList.length > 0 && ser_show">
          <nms-pager-table :data="repairedserverWarningList" :fields="repairedserverWarningFields"  :total-page="serverTotalPage" :biao-zhi="cage" v-model="curPage"/>
        </div>
      </el-tab-pane>
        ...
        <div class="server">
          ...
          <div class="warning">
            <div>
              ...
              <button class='search' @click="searchRepairedWarningsT()"/>
              <span class="clear-filter" @click="clearFilterT()">清除筛选</span>
            </div>
          </div>
        </div>
        <div v-show="repairedterminalWarningList.length === 0 && tlistFlash" class="no-info-tip">
          <span class="PromptImg"></span>
          <span>没有满足条件的统计数据！</span>
        </div>
        <div v-show="repairedterminalWarningList.length > 0 && ter_show">
          <nms-pager-table :data="repairedterminalWarningList" :fields="repairedterminalWarningFields" :total-page="terminalTotalPage" :biao-zhi="cage" v-model="curPage"/>
        </div>
      </el-tab-pane>
...
<script>
    ...
    import {getRepairedServerWarningTblFields,getRepairedTerminalWarningTblFields} from 'assets/js/subwarninginfo';
    ...
    export default {
        ...
        data () {
          return {
            ...
            //已修复服务器告警列表
            repairedserverWarningList: [],
            repairedserverWarningFields: [],
            critical: true,
            important: true,
            normal: true,
            ...
            //已修复终端告警列表
            repairedterminalWarningList: [],
            repairedterminalWarningFields: [],
        ...
        watch: {
          ...
          tabType: function (val) {
            ...
            if(val == "server"){
              api.getLatestServerRepairedWarning({params:{
                  newPageNum: 1,
              ...
              .then((res) => {
                if(res.success==1){
                  this.repairedserverWarningList = res.data
                  this.repairedserverWarningFields = getRepairedServerWarningTblFields()
              ...
              api.getLatestTerminalRepairedWarning({params:{
                  newPageNum: 1,
              ...
              .then((res) => {
                if(res.success==1){
                  ...
                  this.repairedterminalWarningList = res.data
                  this.repairedterminalWarningFields = getRepairedTerminalWarningTblFields()
        ...
        methods:{
          onExport: function () {
            ...
            if (this.tabType == "server") {
              api.getWarningDownLoad({params: {
                ...
                starttime: start_time,
                stoptime: end_time,
                warningType: 'repaired_server_warning'
              }, responseType: 'blob'}).then(res => {
                Upexcele(res, '已修复服务器告警信息.xls')
              }).catch(error => {
                console.log(error)
              ...
              api.getWarningDownLoad({params: {
                ...
                starttime: start_time,
                stoptime: end_time,
                warningType: 'repaired_terminal_warning'
              }, responseType: 'blob'}).then(res => {
                Upexcele(res, '已修复终端告警信息.xls')
              }).catch(error => {
                console.log(error)
                this.errorDialog.open('导出失败')
              });
          ...
          //server
          getRepairedServerWarning: function () {
            let start_time = FormatTime(this.startDate)
            let end_time = FormatTime(this.endDate)
            // server
            api.getLatestServerRepairedWarning({params:{
          ...
          searchRepairedWarningsS: function () {
            var start_time = FormatTime(this.startDate)
            var end_time = FormatTime(this.endDate)
          ...
          //terminal
          getRepairedTerminalWarning: function() {
            let start_time = FormatTime(this.startDate)
            let end_time = FormatTime(this.endDate)
            api.getLatestTerminalRepairedWarning({params:{
                newPageNum: 1,
          ...
          searchRepairedWarningsT: function () {
            var start_time = FormatTime(this.startDate)
            var end_time = FormatTime(this.endDate)
            if (this.timePeriodValue == "lasthour") {
              start_time = start_time

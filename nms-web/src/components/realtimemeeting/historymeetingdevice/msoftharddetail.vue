...
<script>
  ...
  export default {
    ...
    activated: function () {
      ...
      this.deviceMoid = this.$route.params.deviceMoid
      ...
      api.getHistorySoftHardTerminalDetail({params: {meetingMoid: this.meetingRoom, terminalMoid: this.deviceMoid}}).then(res => {
        console.log(res)
        if (res.success == 1) {
          this.detail = res.data
          this.deviceName = this.detail.mt_name
          this.deviceIP = this.detail.mt_ip
          this.deviceE164 = this.detail.mt_e164
          ...
          if (this.OperateList[parseInt(this.detail.operator)]) {
    ...
    methods: {
      meetingback: function() {
        this.$router.push({
          ...
          params: {meetingRoom: this.meetingRoom, meetingType: 'multi'}
        })
      }
    },
    watch: {
      curPage: function (newPage, oldPage) {
        this.cage = 1
        if (this.tabType === 'joinmeeting') {
          this.MeetingLeaveFields = getInOutMeetingFields()
          api.getHistoryTerminalLeaveReason({params: {terminalMoid: this.deviceMoid, meetingMoid: this.meetingRoom, page: newPage}}).then(res => {
      ...
      tabType: function (newTab, oldTab) {
        ...
        if (newTab === 'meetingstate') {
          ...
          api.getHistoryTerminalMeetingScore({params: {terminalMoid: this.deviceMoid, meetingMoid: this.meetingRoom}}).then(res => {
            if (res.success == 1) {
        ...
        if (newTab === 'joinmeeting') {
           // 入离会详情
          this.MeetingLeaveFields = getInOutMeetingFields()
          api.getHistoryTerminalLeaveReason({params: {terminalMoid: this.deviceMoid, meetingMoid: this.meetingRoom}}).then(res => {
            console.log(res)
            if (res.success == 1) {
              if (res.data.length > 0) {
                this.MeetingLeaveData = res.data

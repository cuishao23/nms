...
for i = 1, #Keys do
    ...
    if is_in_table(MachineRoomMoid, MachineRoomMoidList) then
        ...
        if OnlineState ~= nil and OnlineState ~= "offline" and Type ~= "smu" and Type ~= "xmpu" and Type ~= "xmpu5" then
            ...
            local Memory = redis.call("HGET", ResourceKey, "memory")
            if Memory ~= nil then
                Item["memory"] = tonumber(Memory) or 0
                local MemTotal = tonumber(redis.call("HGET", ResourceKey, "memtotal") or '0')
                local MemUsed = tonumber(redis.call("HGET", ResourceKey, "memused") or '0')
                Item["memtotal"] = MemTotal
                Item["memused"] = MemUsed
                Ret[#Ret + 1] = Item
            end
        end
    end

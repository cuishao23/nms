    ...
    for i = 1,table.getn(ServerMoidList) do
        ...
        if redis.call('EXISTS', KeyInfo) == 1 then
            ...
            for i = 1, DiskCount do
                ...
                local DiskTotal = redis.call('HGET',KeyResource,'disk' .. i .. '_total') or ''
                local DiskUsed = redis.call('HGET',KeyResource,'disk' .. i .. '_used') or ''
                local DiskUserate = redis.call('HGET',KeyResource,'disk' .. i .. '_userate') or ''

                -- Format server resource object
                ServerInfo['disk' .. i .. '_name'] = DiskName
                ServerInfo['disk' .. i .. '_total'] = DiskTotal
                ServerInfo['disk' .. i .. '_used'] = DiskUsed
                ServerInfo['disk' .. i .. '_userate'] = DiskUserate
            end 
